...
func NewNDPIClassifier(config NDPIConfig) *NDPIClassifier {
	...
	return &NDPIClassifier{
		...
		rulebook: []ndpiRule{
			...
			{Category: "video", Matches: []string{"rtsp", "rtmp", "hls", "webrtc", "video", "mpeg"}},
			{Category: "audio", Matches: []string{"rtp", "srtp", "rtcp", "opus", "g711", "g729", "audio"}},
			{Category: "control", Matches: []string{"snmp", "ssh", "netconf", "restconf", "bgp", "ospf", "isis", "lldp", "ntp", "radius", "tacacs"}},
		},
	}
}
	...
	for _, rule := range c.rulebook {
		...
		for _, match := range rule.Matches {
			if strings.Contains(lower, match) {
				return NDPIClassification{Protocol: appName, Category: rule.Category}, true
			}
		}
	}
	return NDPIClassification{}, false
